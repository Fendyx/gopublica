...
import { ExternalLink, Wrench, Car, Globe } from 'lucide-react';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'agencyOther' });
  return {
    title: t('title'),
    ...
    openGraph: {
      title: t('title'),
      description: t('description'),
      url: `https://gopublica.com/${locale}/agency-other`,
      locale,
      type: 'website',
    },
  };
...
export default function OtherPage() {
  const t = useTranslations('agencyOther');
    ...
    <div className="relative w-full">
      ...
      <div className="sticky left-0 top-0 z-0 h-screen w-full overflow-hidden">
        <video
          ...
          poster="/videos/other-poster.jpg"
        >
          <source src="/videos/other-demo.mp4" type="video/mp4" />
        </video>
        ...
        <div className="overflow-hidden rounded-t-[2.5rem] bg-[var(--surface)] shadow-[0_-8px_60px_rgba(0,0,0,0.18)]">
          ...
          <section className="mx-auto max-w-5xl px-6 py-20">
            ...
            <div className="grid gap-5 md:grid-cols-3">
              {(['carService', 'tuning', 'anyBusiness'] as const).map((feat) => {
